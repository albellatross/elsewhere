...
import Card04Detail from './components/Card04Detail';
import RemixIdeaDetailPage from './components/RemixIdeaDetailPage';
import LoadingOverlay from './components/LoadingOverlay';
import StudioComingSoon from './components/StudioComingSoon';
import { remixIdeas } from './components/remixIdeasConfig';
...
export default function UserApp() {
  ...
  const [activeTopSection, setActiveTopSection] = useState<'home' | 'studio'>('home');
  ...
  const detailPageIds: string[] = [
    ...
    'pet-portrait',
    'product-visuals',
    ...remixIdeas.map((idea) => idea.id)
  ];

  const pickRandomDetailPage = (current: string | null) => {
    const options = detailPageIds.filter((id) => id !== current);
    const pool = options.length > 0 ? options : detailPageIds;
    const index = Math.floor(Math.random() * pool.length);
    return pool[index] ?? detailPageIds[0];
  };

  const handleTryItNow = () => {
    setActiveDetailPage(pickRandomDetailPage(activeDetailPage));
  };
  ...
  // 增加平滑滚动配置，让视差效果更有“呼吸感”
  const smoothProgress = useSpring(scrollYProgress, {
    stiffness: 70, // 降低刚度，增加柔和感
    damping: 30,   // 适中的阻尼
    restDelta: 0.001
  });
  ...
  // 首页隐藏滚动条，详情页保留自身滚动条样式
  useEffect(() => {
    const shouldHideScrollbar = activeDetailPage === null;
    const targets = [document.documentElement, document.body];
    targets.forEach((el) => el.classList.toggle('scrollbar-hide', shouldHideScrollbar));
    return () => {
      targets.forEach((el) => el.classList.remove('scrollbar-hide'));
    };
  }, [activeDetailPage]);

  // 视差效果配置 - 使用 smoothProgress 替代 scrollYProgress 以获得更丝滑的体验
  const bgX = useTransform(smoothProgress, [0, 1], ['0vw', '12vw']);
  const bgY = useTransform(smoothProgress, [0, 1], ['0vh', '12vh']);

  // 背景图片透明度 - 增加非线性变化，创造呼吸节奏
  const bgOpacity = useTransform(smoothProgress, [0, 0.4, 0.8, 1], [1, 0.85, 0.5, 0.2]);

  // 森林背景视差
  const forestX = useTransform(smoothProgress, [0, 1], ['0vw', '4vw']);

  // 人物图片视差 - 响应式优化，增加一点缩放呼吸
  const portraitY = useTransform(smoothProgress, [0, 1], ['0vh', '-18vh']);
  const portraitX = useTransform(smoothProgress, [0, 1], ['0vw', '-15vw']);
  const portraitScale = useTransform(smoothProgress, [0, 1], [0.92, 1]);

  // 卡片01动画 - 增加偏移量和非线性时序，创造穿插感
  const card01Rotate = useTransform(smoothProgress, [0, 0.6, 1], [11, 28, 45]);
  const card01Y = useTransform(smoothProgress, [0, 0.6, 1], ['0vh', '-12vh', '-25vh']);
  const card01X = useTransform(smoothProgress, [0, 0.6, 1], ['0vw', '4vw', '10vw']);
  const card01Scale = useTransform(smoothProgress, [0, 1], [0.9, 0.96]);

  // 卡片02动画 - 调整为向上移动，消除“掉落感”，保持空间错落
  const card02Rotate = useTransform(smoothProgress, [0, 0.5, 1], [-14, -22, -35]);
  const card02Y = useTransform(smoothProgress, [0, 0.5, 1], ['0vh', '-4vh', '-10vh']);
  const card02X = useTransform(smoothProgress, [0, 0.5, 1], ['0vw', '-3vw', '-8vw']);
  const card02Scale = useTransform(smoothProgress, [0, 1], [0.9, 0.96]);

  // 卡片03动画 - 强调前景感，大幅度位移
  const card03Rotate = useTransform(smoothProgress, [0, 0.4, 1], [28, 45, 65]);
  const card03Y = useTransform(smoothProgress, [0, 0.4, 1], ['0vh', '-18vh', '-40vh']);
  const card03X = useTransform(smoothProgress, [0, 0.4, 1], ['0vw', '3vw', '8vw']);
  const card03Scale = useTransform(smoothProgress, [0, 1], [0.95, 1.15]);

  // 装饰线条动画 - 增加轻微漂浮感
  const decorLineRotate = useTransform(smoothProgress, [0, 1], [0, 5]);
  const decorLineScale = useTransform(smoothProgress, [0, 1], [1, 1.05]);
  const decorLineOpacity = useTransform(smoothProgress, [0, 0.3, 0.55], [1, 0.3, 0]);
  const decorLineY = useTransform(smoothProgress, [0, 1], ['0vh', '-35vh']);
  const decorLineX = useTransform(smoothProgress, [0, 1], ['0vw', '15vw']);

  // 底部装饰线动画
  const bottomLineOpacity = useTransform(smoothProgress, [0, 0.3, 0.6], [1, 0.3, 0]);
  const bottomLineScale = useTransform(smoothProgress, [0, 1], [1, 1.05]);
  const bottomLineY = useTransform(smoothProgress, [0, 1], ['0vh', '-35vh']);
  const bottomLineX = useTransform(smoothProgress, [0, 1], ['0vw', '12vw']);

  // 文字内容淡出 - 更加自然的过渡
  const textOpacity = useTransform(smoothProgress, [0, 0.35], [1, 0]);
  const textY = useTransform(smoothProgress, [0, 0.35], ['0vh', '-15vh']);

  // 滚动提示动画
  const scrollHintOpacity = useTransform(smoothProgress, [0, 0.15], [1, 0]);
  const scrollHintY = useTransform(smoothProgress, [0, 0.15], ['0vh', '-2vh']);

  // 导航栏交互逻辑
  const navHeroOpacity = useTransform(smoothProgress, [0, 0.25, 0.35], [1, 0.4, 0]);
  const navHeroY = useTransform(smoothProgress, [0, 0.35], ['0vh', '-12vh']);

  // 移动端视差效果 - 同样应用平滑逻辑
  const mobileContainerY = useTransform(smoothProgress, [0, 0.5], ['0vh', '-12vh']);
  const mobilePortraitY = useTransform(smoothProgress, [0, 0.5], ['0vh', '-8vh']);
  const mobilePortraitScale = useTransform(smoothProgress, [0, 0.5], [0.92, 0.98]);
  const mobileCard01Y = useTransform(smoothProgress, [0, 0.5], ['0vh', '-5vh']);
  const mobileCard01Rotate = useTransform(smoothProgress, [0, 0.5], [8, 18]);
  const mobileCard01Scale = useTransform(smoothProgress, [0, 0.5], [0.9, 0.95]);
  const mobileCard02Y = useTransform(smoothProgress, [0, 0.5], ['0vh', '4vh']);
  const mobileCard02Rotate = useTransform(smoothProgress, [0, 0.5], [-10, -22]);
  const mobileCard02Scale = useTransform(smoothProgress, [0, 0.5], [0.9, 0.95]);
  const mobileCard03Y = useTransform(smoothProgress, [0, 0.5], ['0vh', '-10vh']);
  const mobileCard03Rotate = useTransform(smoothProgress, [0, 0.5], [20, 40]);
  const mobileCard03Scale = useTransform(smoothProgress, [0, 0.5], [0.9, 0.98]);
  ...
  const isStudioView = activeTopSection === 'studio';

  const navigateToSection = (ref: React.RefObject<HTMLDivElement>) => {
    if (activeTopSection !== 'home') {
      setActiveTopSection('home');
      window.requestAnimationFrame(() => {
        window.requestAnimationFrame(() => scrollToSection(ref));
      });
      return;
    }
    scrollToSection(ref);
  ...
  const headerMotionStyle = isStudioView ? { opacity: 1, y: 0 } : { opacity: navHeroOpacity, y: navHeroY };
  const studioButtonAnimate = isStudioView
    ? { backgroundColor: 'rgba(255,255,255,1)', scale: 1.05 }
    : { backgroundColor: 'rgba(255,255,255,0)', scale: 1 };
  const studioIconAnimate = isStudioView ? { fill: '#7160E8' } : { fill: '#050505' };
  const studioTextAnimate = isStudioView ? { color: '#7160E8' } : { color: '#050505' };

  const HeaderBar = () => (
    <>
      {/* Logo - 首页时跟随滚动，Studio 页面固定 */}
      <motion.div
        className="absolute h-[3.5vh] left-[5vw] md:left-[6.88vw] top-[3.5vh] w-[5.5vw] min-w-[70px] z-50 cursor-pointer"
      ...
      >
        ...
        <motion.div
          ...
          whileHover="hover"
          whileTap="tap"
          onClick={() => navigateToSection(templatesSectionRef)}
        ...
        >
          <motion.div
            ...
            variants={{
              ...
              hover: { backgroundColor: 'rgba(255,255,255,1)', scale: 1.05 },
              tap: { backgroundColor: 'rgba(255,255,255,1)', scale: 0.95 }
            }}
          />
          <motion.div
          ...
          <motion.p
            className="font-sans font-semibold leading-[normal] relative shrink-0 text-[clamp(12px,0.83vw,16px)] text-nowrap"
            variants={{
        ...
        <motion.div
          ...
          whileHover="hover"
          whileTap="tap"
          onClick={() => navigateToSection(remixIdeasRef)}
          transition={{ duration: 0.2 }}
        >
          <motion.div
            className="absolute inset-0"
        ...
        <motion.div
          className="content-stretch flex gap-[0.52vw] h-[4.07vh] items-center justify-center pl-[1.04vw] pr-[1.25vw] py-[1.11vh] relative rounded-[999px] shrink-0 w-[7.81vw] min-w-[120px] cursor-pointer overflow-hidden"
          initial="initial"
          animate={isStudioView ? "active" : "initial"}
          whileHover="hover"
          whileTap="tap"
          onClick={() => setActiveTopSection('studio')}
          transition={{ duration: 0.2 }}
          aria-current={isStudioView ? "page" : undefined}
        >
          <motion.div
            className="absolute inset-0"
            variants={{
              initial: { backgroundColor: 'rgba(255,255,255,0)', scale: 1 },
              active: { backgroundColor: 'rgba(255,255,255,1)', scale: 1.05 },
              hover: { backgroundColor: 'rgba(255,255,255,1)', scale: 1.05 },
              tap: { backgroundColor: 'rgba(255,255,255,1)', scale: 0.95 }
            }}
          />
  ...
  return (
    <>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeDetailPage === 'product-visuals' && (
          <Card04Detail onClose={() => setActiveDetailPage(null)} />
        )}
        {activeDetailPage?.startsWith('remix-') && (
          <RemixIdeaDetailPage
            ideaId={activeDetailPage}
            onClose={() => setActiveDetailPage(null)}
            onSelectIdea={(ideaId) => setActiveDetailPage(ideaId)}
          />
        )}
      </AnimatePresence>
      ...
      {/* 主页面内容 */}
      {isStudioView ? (
        <div className="w-full bg-white relative min-h-screen overflow-hidden" style={{ position: 'relative' }}>
          <div className="relative w-full min-h-screen">
            <HeaderBar />
            <div className="flex items-center justify-center min-h-screen">
              <StudioComingSoon className="w-full" />
            </div>
          </div>
        </div>
      ) : (
        <div className="w-full bg-white relative min-h-screen" style={{ position: 'relative' }}>
            ...
            <div className="sticky top-0 w-full h-screen overflow-x-clip overflow-y-visible relative">
              <div className="bg-white h-full w-full relative">
                ...
                >
                  ...
                  {/* 右上装饰线条 - 仅桌面版 */}
                  <motion.div
                    className="hidden md:block absolute h-[6.86vh] left-[149vw] top-[42vh] w-[20.16vw] max-w-[387px]"
                    style={{
                  ...
                  {/* 卡片 02 */}
                  <motion.div
                    className="absolute flex items-center justify-center left-[37vw] md:left-[108vw] top-[68vh] md:top-[30vh] w-[25vw] md:w-[20vw] z-20"
                    style={{
                      y: card02Y,
                      x: card02X,
                      rotate: card02Rotate,
                ...
                <motion.div
                  className="absolute left-0 top-[14vh] md:top-[53%] md:translate-y-[-50%] md:translate-x-[-2vw] w-full z-30"
                  style={{
                    opacity: textOpacity,
                    y: textY,
                    willChange: 'transform, opacity'
                      ...
                      >
                        <div className="content-stretch flex flex-col gap-[clamp(36px,5.8vh,66px)] items-center md:items-start relative shrink-0">
                          <div className="content-stretch flex flex-col gap-[clamp(8px,1.2vh,14px)] items-center md:items-start relative shrink-0">
                            ...
                            >
                              <p className="font-nord font-bold leading-[0.9] relative shrink-0 text-[#0b0b0b] 
                            ...
                            <motion.p
                              className="font-nord font-medium leading-[0.9] relative shrink-0 text-[#8e8e93] text-center md:text-left 
                          ...
                          >
                            <p className="font-nord font-semibold leading-[1.2] relative shrink-0 text-[#1c1c1e] text-center md:text-left 
                                        text-[clamp(14px,3.8vw,17px)] md:text-[clamp(16px,1.3vw,19px)] lg:text-[clamp(18px,1.4vw,20px)]">
                              Same you. New worlds.
                            </p>
                            <div className="font-nord font-normal leading-[1.35] relative shrink-0 text-[#5f5f66] text-center md:text-left 
                        ...
                        >
                          ...
                          <p className="font-sans font-semibold leading-[normal] relative shrink-0 
                ...
                >
                  ...
                  <p className="font-sans font-normal leading-[normal] relative shrink-0 text-[#242424] text-[clamp(12px,0.73vw,14px)] text-nowrap">Scroll to explore more</p>
                  <motion.div
          ...
          <div ref={remixIdeasRef}>
            <RemixIdeasSection onCardClick={(cardId) => setActiveDetailPage(cardId)} />
          </div>
          <Footer />
        </div>
      )}
